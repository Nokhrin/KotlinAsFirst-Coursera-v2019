...
fun firstDuplicateIndex(str: String): Int {
    // приводим строку к нижнему регистру, разбираем строку на список слов
    val inputList = str.toLowerCase().split(" ").toList()
    // проходим подряд по списку, сравниваем слова
    for (i in 0 until inputList.size - 1) { // исключаем последний элемент, чтобы не выйти за границы списка
        // определяем индекс повторяющегося слова, до которого нужно посчитать символы
        if (inputList[i] == inputList[i + 1]) {
            var letterIndex = 0
            // считаем количество символов в словах до найденного и добавляем количество пробелов
            for (j in 0 until i) {
                letterIndex += inputList[j].length
            }
            // добавляем пробелы (их количество равно количеству слов)
            letterIndex += i
            return letterIndex
        }
    }
...
fun mostExpensive(description: String): String {
    ...
    if (regex.matches(description)) {
        ...
        // если встретили отрицательное число, останавливаем выполнение
        val inputList = description.split("; ", " ").toList()
        val preparedList = inputList.withIndex() // превращаем список в коллекцию вида "индекс - объект"
            .groupBy { it.index / 2 } // группируем объекты попарно - частное индекса и 2 округляется до целого
            // получаем 1/2 = 2/2 - группа 1, 3/2 = 4/2 - группа 2 и так далее
