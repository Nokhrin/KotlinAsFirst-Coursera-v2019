...
fun countSubstrings(inputName: String, substrings: List<String>): Map<String, Int> {
    val stringsFound = mutableMapOf<String, Int>()
    // читаем файл, приводим к нижнему регистру, записываем в переменную
    val contents = File(inputName).readText().toLowerCase()
    substrings.forEach { substring ->
        var counter = 0
        // просматриваем содержимое по кусочкам длиной равной длине искомой строки
        for (i in 0..contents.length - substring.length) {
            if (contents.slice(i until i + substring.length) == substring.toLowerCase()) {
                counter += 1
            }
        }
        stringsFound[substring] = counter
    }
    return stringsFound
}
...
fun sibilants(inputName: String, outputName: String) {
    // читаем текст
    val textCheck = File(inputName).readText().toMutableList()
    // находим и запоминаем ошибки
    // регистр вида "индекс символа - символ"
    val corrected = mutableMapOf<Int, Char>()
    for (i in 0 until textCheck.size - 1) {
        if (textCheck[i].toLowerCase() == 'ж' ||
            textCheck[i].toLowerCase() == 'ч' ||
            textCheck[i].toLowerCase() == 'ш' ||
            textCheck[i].toLowerCase() == 'щ'
        ) {
            // если встретили букву, то внесли в регистр и заменили
            if (textCheck[i + 1] == 'ы') {
                corrected[i + 1] = textCheck[i + 1]
                textCheck[i + 1] = 'и'
            }
            if (textCheck[i + 1] == 'Ы') {
                corrected[i + 1] = textCheck[i + 1]
    ...
    val outputStream = File(outputName).bufferedWriter()
    outputStream.write(textCheck.joinToString(""))
    outputStream.close()
}
